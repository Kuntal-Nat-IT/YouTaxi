...
def GetAllCarsForAdmin(request):
    try:
        CarObjectList = Car.objects.all().filter(isDeleted=False)
...
def CarTypeById(request, slug):
    try:
        ...
        CarTypeObject = CarTypeList.data
        ack = 5
        success = True
        status = 200
        msg = "Car Loaded"
    ...
    data = {'success': success, 'status': status, 'ack': ack, 'msg': msg, 'UserData': CarTypeObject}
    return Response(data=data, status=status)
...
def CarUpdate(request,slug):
    try:
        ...
        if CarUpdate.is_valid(raise_exception=True):
            CarUpdate.save()
            ack = 5
            success = True
            status = 201
...
@api_view(['POST'])
def ChangedCarStatus(request,slug):
    try:
        CarObject = Car.objects.get(id=slug)
        if CarObject.status:
            CarObject.status = False
        else:
            CarObject.status = True
        CarObject.save()
        ...
        print("ChangedCarStatus : ", e)
        success = False
        status = 400
        ack = 1
        msg = "Fail To Status Change"
...
@api_view(['POST'])
def MakeCareActivateDeactivate(request,slug):
    try:
        CarObject = Car.objects.get(id=slug)
        CarObject.isDeleted = True
        CarObject.save()
