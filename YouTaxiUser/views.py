...
from YouTaxiAdmin.models import UserBooking
# Geo Lib
...
@api_view(['POST'])
def taxiBooking(request):
    try:
        origin = [request.data['origin']['latitude'], request.data['origin']['longitude']]
        destination = [request.data['destination']['latitude'], request.data['destination']['longitude']]
        taxiType = request.data('taxiType')
        user = request.data('user')
        UserBooking(user=user, taxiType=taxiType, originLatitude=origin[0], originLongitude=origin[1], destinationLatitude=destination[0], destinationLongitude=destination[1]).save()
        ack = 5
    except Exception as e :
        ack=1
        msg='fail'
